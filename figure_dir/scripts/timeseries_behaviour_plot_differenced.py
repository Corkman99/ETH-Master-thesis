...
auto_corrs = auto_correlation(data, max_lag=lags)
cross_corrs = cross_correlation(data, max_lag=lags)

x_auto = np.arange(0,2*lags+1,1)
x_cross = np.arange(-lags,lags+1,1)

colors = ['#4B9C50','#D1BA48','#D36159','#5994D3']

# Plot Auto-Correlation
bar_width = 0.2

labels = ['T\'', 'Adv T\'', 'Adiab T\'', 'Diab T\'']
for i in range(auto_corrs.shape[1]):
    ax_auto.bar(x_auto + i * 0.2, auto_corrs[:, i], width=bar_width, label=labels[i],color = colors[i])

ax_auto.text(0.025, 0.95, r'(e) Average $\hat{\rho}_{k}$', ha='left', va='top', transform=ax_auto.transAxes,
            bbox=dict(facecolor='white', edgecolor='black'))
ax_auto.set_xlabel(r'Lag $k$ (days)')
ax_auto.set_ylabel(r'$\hat{\rho}_{k}$')
ax_auto.set_xlim((-1,2*lags+1))
ax_auto.set_xticks([0,8,16,24,32])
ax_auto.set_xticklabels(['0','1','2','3','4'])
ax_auto.legend()

for i in range(cross_corrs.shape[1]):
    ax_cross.bar(x_cross + i * 0.2, cross_corrs[:, i], width=bar_width, label=f'T\' and {labels[i+1]}', color=colors[i+1])

ax_cross.text(0.025, 0.95, r'(e) Average $\hat{r}_k(T^\prime,X)$', ha='left', va='top', transform=ax_cross.transAxes,
            bbox=dict(facecolor='white', edgecolor='black'))
ax_cross.set_xlabel(r'Lag $k$ (days)')
ax_cross.set_ylabel(r'$\hat{r}_k(T\prime,X)$')
ax_cross.set_xticks([-16,-8,0,8,16])
ax_cross.set_xticklabels(['-2','-1','-0','1','2'])

# Adjust layout for better spacing
plt.tight_layout()

# Adjust layout
plt.savefig(f'{outdir}timeseries_lat{lat}_lon{lon}_diff.png') #, bbox_inches='tight')
plt.close()
